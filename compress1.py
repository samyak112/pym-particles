...
from data_processing import load_bitmap,build_ends_index,build_runs_list
...
def step_model_batched(model, tokens_batch, global_positions=None, runs_list=None, run_cursors=None, past_kv=None, past_padding=None):
    ...
    with torch.no_grad():
        ...
        # logits[:, -1, :] is the prediction for each stream
        last_logits = logits[:, -1, :].float()  # (N, vocab)

        if global_positions is not None and runs_list is not None:
            for b in range(last_logits.shape[0]):
                gpos = global_positions[b]
                # advance this stream's cursor
                while run_cursors[b] < len(runs_list) and runs_list[run_cursors[b]][1] < gpos:
                    run_cursors[b] += 1
                if run_cursors[b] < len(runs_list):
                    start, end, mask = runs_list[run_cursors[b]]
                    if start <= gpos <= end:
                        last_logits[b, :256].masked_fill_(mask.to(DEVICE), float('-inf'))
...
def decompress(model, compressed_path, custom_seeds, output_path, vocab_size=258):
    ...
    for block_idx in tqdm(range(num_blocks), desc="decompress", unit="block"):
        ...
        run_cursors = [0] * num_chunks  # before block loop
...
if __name__ == '__main__':
    ...
    RECONSTRUCTED   = 'slice_100mb.reconstructed_current.txt'
    SEED_FILE       = 'seeds.bin'
